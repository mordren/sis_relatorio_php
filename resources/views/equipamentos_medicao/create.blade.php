...
<div class="container mx-auto px-4 py-8 max-w-2xl">
    <h1 class="text-3xl font-bold mb-6">Novo Equipamento de Medição</h1>
    ...
    <form method="POST" action="{{ route('equipamentos-medicao.store') }}" class="bg-white shadow rounded-lg p-6">
        @csrf
        ...
        <div class="mb-6">
            ...
            <input type="text" name="numero_serie" id="numero_serie" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 @error('numero_serie') border-red-500 @enderror" value="{{ old('numero_serie') }}" required>
            @error('numero_serie')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <input type="date" name="data_calibracao" id="data_calibracao" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 @error('data_calibracao') border-red-500 @enderror" value="{{ old('data_calibracao') }}">
        ...
        <div class="mb-6">
            ...
            <textarea name="observacao" id="observacao" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 @error('observacao') border-red-500 @enderror">{{ old('observacao') }}</textarea>
            @error('observacao')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            <label for="ativo" class="flex items-center">
                <input type="checkbox" name="ativo" id="ativo" class="rounded" value="1" {{ old('ativo', true) ? 'checked' : '' }}>
